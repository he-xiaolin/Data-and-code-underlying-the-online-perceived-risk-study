...
def shap_cal(model_name, data, K, event_id, event_duration, event_numb, feature_names, per_event=False):
    ...
    try:
        ...
        plt.figure(figsize=(6, 4))
        plt.barh(range(topk), vals, color=essential_pink)
...
# ---- CLI arguments ----
parser = argparse.ArgumentParser(description='SHAP for trained MC-Dropout model (MB)')
parser.add_argument('--scenario', type=str, default='MB', choices=['HB', 'MB', 'SVM', 'LC_1', 'LC_2', 'LC_3'])
...
# MB feature names (21 vars; fallback handled inside shap_cal if mismatch)
feature_names = [
    "$v_{x,s}$",
    "$v_{x,n}$",
    "$v_{y,n}$",
