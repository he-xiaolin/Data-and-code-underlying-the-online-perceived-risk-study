% This function is to calibrate DRF and PCAD in SVM scenario. 
clear;clc;close all;

% add path and load data
addpath('Model_Functions/DRF');
addpath('Model_Functions/PCAD');
...
if isempty(poolObj)
    ...
    % There is an existing pool. You can decide what to do:
...
vx_s = [SVM.vx_s];
vx_n = [SVM.vx_n];
vy_s = [SVM.vy_s];vy_s(:)=0;
vy_n = [SVM.vy_n];vy_n(:)=0;
ax_s = [SVM.ax_s];
ax_n = [SVM.ax_n];
ay_s = [SVM.ay_s];
ay_n = [SVM.ay_n];
...
PR = [SVM_PR.risk_p25_p75_mean];% No warped
len = length(x_s);
...
%% save
save('./CalibratedParameters_SVM','results_DRF','results_PCAD','DRF','PCAD');