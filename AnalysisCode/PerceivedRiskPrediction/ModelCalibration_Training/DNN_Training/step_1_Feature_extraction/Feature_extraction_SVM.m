...
mkdir('processed_feature/SVM/regression_input');
mkdir('processed_feature/SVM/regression_label');
data_ = load(pt);
data = data_.SVM;
for i=1:27 %   SVM event order 1-27
% s is ego
...
File_name =append('processed_feature/SVM/regression_input/SVM_input_',num2str(i,'%02d'),'.mat');
save(File_name,'Time', ...
        'vx_s', 'vx_n', 'vx_n2',...
        'ax_s' ,'ax_n' , 'ax_n2',...
        'vy_s' ,'vy_n' , 'vy_n2',...
...
j= 0;
for i = 1:27
    SVM_rating = SVM_PR(i).risk_p25_p75_mean;
    File_name =append('processed_feature/SVM/regression_label/SVM_rating_',num2str(i,'%02d'),'.mat');
    save(File_name,'SVM_rating');
end
disp('SVM Done!')
