...
Input_Dic_name = '../../step_1_Feature_extraction/processed_feature/HB/regression_input/'
Label_Dic_name = '../../step_1_Feature_extraction/processed_feature/HB/regression_label/'

Input_File_name_list = os.listdir(Input_Dic_name)
Input_File_name_list.sort()
Input_File_name_list= [ i for i in Input_File_name_list if 'HB' in i ]

Label_File_name_list = os.listdir(Label_Dic_name)
Label_File_name_list.sort()
Label_File_name_list= [ i for i in Label_File_name_list if 'HB' in i ]
...
for ind, File_name in enumerate(Input_File_name_list):  # event files   1-27
    ...
    Label = np.transpose(mat_file_label['HB_rating'])
  ...
  # input feature      
    ...
    HB_feature_reg.append(comb_temp)
        
    
feature_len = comb_temp.shape[-1]
print('HB_feature Type:',type(HB_feature_reg))
HB_feature_reg = np.array(HB_feature_reg).reshape(-1,feature_len)
np.save('../data/HB_feature_reg.npy',HB_feature_reg)
