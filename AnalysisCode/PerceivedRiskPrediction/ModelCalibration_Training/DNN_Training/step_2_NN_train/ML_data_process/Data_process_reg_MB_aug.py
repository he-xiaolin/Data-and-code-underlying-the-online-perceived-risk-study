...
Input_File_name_list= [ i for i in Input_File_name_list if 'MB' in i ]

Label_File_name_list = os.listdir(Label_Dic_name)
Label_File_name_list.sort()
Label_File_name_list= [ i for i in Label_File_name_list if 'MB' in i ]
...
for ind, File_name in enumerate(Input_File_name_list):  # event files   1-27
    ...
    Label = np.transpose(mat_file_label['MB_rating'])
...
# newly added delta v, delta a
  ...
  # input feature   MB    
    ...
    MB_feature_reg.append(comb_temp)
      
feature_len = comb_temp.shape[-1]
print('MB_feature Type:',type(MB_feature_reg))
MB_feature_reg = np.array(MB_feature_reg).reshape(-1,feature_len)
np.save('../data/MB_feature_reg_aug.npy',MB_feature_reg)
